...
return new class extends Migration
{
    ...
    {
        Schema::create('documento_mantenimiento', function (Blueprint $table) {
            $table->id();
            $table->foreignId('asignacion_id')->constrained('asignacion_operador_unidad')->onDelete('restrict');
            $table->string('tecnologia_reportada');
            $table->date('fecha');
            $table->time('hora');
            ...
            $table->foreignId('firma_tabulacion_id')->nullable()->constrained('firmas_digitales')->onDelete('set null');
    ...
    {
        Schema::dropIfExists('documento_mantenimiento');
    }
};